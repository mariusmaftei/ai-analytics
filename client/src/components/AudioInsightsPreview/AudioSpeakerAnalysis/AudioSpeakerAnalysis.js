...
const AudioSpeakerAnalysis = ({ data, rawText, analysisData }) => {
  const parsedData = useMemo(() => {
    ...
    console.log("[AudioSpeakerAnalysis] Raw sections:", sections);
    console.log("[AudioSpeakerAnalysis] Raw text length:", text.length);
    if (text) {
      console.log("[AudioSpeakerAnalysis] Raw text preview:", text.substring(0, 500));
    }

    // Parse Speaker Overview
    const overviewSection = sections.find(
      (s) => s.name?.toLowerCase().includes("speaker overview")
    );
    ...
    if (overviewSection) {
      ...
      items.forEach((item) => {
        ...
        const speakerCountMatch = itemText.match(/number\s+of\s+speakers[:\s]*(\d+)/i);
    ...
    const timeDistributionSection = sections.find(
      (s) => s.name?.toLowerCase().includes("speaking time distribution")
    );
    ...
    const timelineSection = sections.find(
      (s) => s.name?.toLowerCase().includes("speaker timeline")
    );
    ...
    const patternsSection = sections.find(
      (s) => s.name?.toLowerCase().includes("conversation patterns")
    );
    ...
    const breakdownSection = sections.find(
      (s) => s.name?.toLowerCase().includes("speaker breakdown")
    );
      ...
      items.forEach((item) => {
        ...
        if (breakdownMatch) {
          result.speakerBreakdown.push({
            ...
            totalTime: `${breakdownMatch[2]}:${breakdownMatch[3].padStart(2, "0")}`,
    ...
    if (text) {
      console.log("[AudioSpeakerAnalysis] Parsing from rawText");
      console.log("[AudioSpeakerAnalysis] Full rawText:", text);
      ...
      if (timeDistMatch) {
        ...
        console.log("[AudioSpeakerAnalysis] Found Speaking Time Distribution:", timeDistText.substring(0, 500));
        ...
        const speakerPattern = /(Speaker\s+[A-Z][^:]*?|[^:]+?):\s*(\d+):(\d+):\s*([\d.]+)%(?::\s*(\d+)\s+words:\s*(\d+)\s+segments)?/gi;
        ...
        if (matches.length > 0) {
          matches.forEach((match) => {
            ...
            speakerLabel = speakerLabel.replace(/\s*\d+\s*segments?\s*$/i, "").trim();
            ...
            speakerLabel = speakerLabel.replace(/\d+\s*segments?(?=Speaker)/i, "").trim();
            ...
            console.log("[AudioSpeakerAnalysis] Added speaker (pattern match):", speakerLabel, "Percentage:", match[4]);
          });
        } else {
          // Fallback: Try splitting by newlines
          const lines = timeDistText.split(/\n/).filter(line => line.trim().length > 0);
          lines.forEach((line) => {
            console.log("[AudioSpeakerAnalysis] Processing line:", line);
            ...
            if (speakerMatch) {
              ...
              console.log("[AudioSpeakerAnalysis] Added speaker:", speakerLabel);
            } else {
              ...
              if (speakerMatch2) {
                ...
                console.log("[AudioSpeakerAnalysis] Added speaker (simple):", speakerLabel);
              }
            }
          });
        }
      } else {
        console.log("[AudioSpeakerAnalysis] No Speaking Time Distribution section found");
        // Try to extract speakers from breakdown if available
        if (result.speakerBreakdown.length > 0) {
          console.log("[AudioSpeakerAnalysis] Extracting speakers from breakdown data");
          result.speakerBreakdown.forEach((breakdown) => {
            // Check if speaker already exists
            const existingSpeaker = result.speakers.find(s => s.label === breakdown.speaker);
            if (!existingSpeaker) {
              // Parse time from breakdown
              const [minutes, seconds] = breakdown.totalTime.split(":").map(Number);
              // Clean speaker label - remove any trailing numbers or "segments" text
              ...
              console.log("[AudioSpeakerAnalysis] Added speaker from breakdown:", speakerLabel);
            }
          });
        }
      }
      ...
      const timelineMatch = text.match(
        /SECTION:\s*Speaker\s+Timeline\s*[:\n]*([\s\S]*?)(?=SECTION:|$)/i
      );
      if (timelineMatch) {
        const timelineText = timelineMatch[1];
        console.log("[AudioSpeakerAnalysis] Found Speaker Timeline:", timelineText.substring(0, 500));
        ...
        const timelinePattern = /(\d{1,2}:\d{2})-(\d{1,2}:\d{2}):\s*(Speaker\s+[A-Z][^:]*?|[^:]+?):\s*((?:(?!\d{1,2}:\d{2}-\d{1,2}:\d{2}).)+?)(?=\d{1,2}:\d{2}-\d{1,2}:\d{2}|$)/gi;
        ...
        if (matches.length > 0) {
          matches.forEach((match) => {
            ...
            console.log("[AudioSpeakerAnalysis] Added timeline item:", match[1], "-", match[2], "Speaker:", match[3].trim());
          });
        } else {
          ...
          lines.forEach((line) => {
            ...
            if (timelineItemMatch) {
              ...
              console.log("[AudioSpeakerAnalysis] Added timeline item (line):", timelineItemMatch[1], "-", timelineItemMatch[2]);
            } else {
        ...
        console.log("[AudioSpeakerAnalysis] No Speaker Timeline section found");
      }

      // Parse Conversation Patterns from rawText
      const patternsMatch = text.match(
        /SECTION:\s*Conversation\s+Patterns\s*[:\n]*([\s\S]*?)(?=SECTION:|$)/i
      );
      if (patternsMatch) {
        const patternsText = patternsMatch[1];
        console.log("[AudioSpeakerAnalysis] Found Conversation Patterns:", patternsText.substring(0, 500));
        ...
        if (lines.length > 0) {
          lines.forEach((line) => {
            ...
            if (cleanLine.length > 0 && !cleanLine.startsWith("SECTION:")) {
              ...
              if (cleanLine.includes("- ") && !cleanLine.match(/\d{1,2}:\d{2}-\d{1,2}:\d{2}/)) {
                ...
                subPatterns.forEach((pattern) => {
                  ...
                  if (trimmed.length > 0) {
                    ...
                    console.log("[AudioSpeakerAnalysis] Added pattern (split):", trimmed);
                  }
                });
              } else {
                result.conversationPatterns.push(cleanLine);
                console.log("[AudioSpeakerAnalysis] Added pattern:", cleanLine);
              }
            }
          });
        } else {
          ...
          splitPatterns.forEach((pattern) => {
            ...
            if (trimmed.length > 0 && !trimmed.startsWith("SECTION:")) {
              result.conversationPatterns.push(trimmed);
              console.log("[AudioSpeakerAnalysis] Added pattern (no newlines):", trimmed);
            }
          });
        }
      } else {
        console.log("[AudioSpeakerAnalysis] No Conversation Patterns section found");
      }

      // Parse Speaker Breakdown from rawText
      const breakdownMatch = text.match(
        /SECTION:\s*Speaker\s+Breakdown\s*[:\n]*([\s\S]*?)(?=SECTION:|$)/i
      );
      if (breakdownMatch) {
        const breakdownText = breakdownMatch[1];
        console.log("[AudioSpeakerAnalysis] Found Speaker Breakdown:", breakdownText.substring(0, 500));
        ...
        const breakdownPattern = /(Speaker\s+[A-Z][^:]*?|[^:]+?):\s*(\d+):(\d+):\s*([\d.]+)%:?\s*(\d+):?\s*([\d.]+)\s+sec:\s*((?:(?!Speaker\s+[A-Z]|[A-Z][^:]*?:\s*\d+:\d+).)+?)(?=Speaker\s+[A-Z]|[A-Z][^:]*?:\s*\d+:\d+|$)/gi;
        ...
        if (matches.length > 0) {
          matches.forEach((match) => {
            ...
            console.log("[AudioSpeakerAnalysis] Added breakdown (pattern match):", speakerLabel, "Percentage:", match[4]);
          });
        } else {
          // Fallback: Try splitting by newlines
          ...
          lines.forEach((line) => {
            ...
            if (breakdownItemMatch) {
              ...
              result.speakerBreakdown.push({
                speaker: speakerLabel,
                totalTime: `${breakdownItemMatch[2]}:${breakdownItemMatch[3].padStart(2, "0")}`,
              ...
              console.log("[AudioSpeakerAnalysis] Added breakdown:", speakerLabel);
            }
          });
        }
      } else {
        console.log("[AudioSpeakerAnalysis] No Speaker Breakdown section found");
      }
    }
    ...
    console.log("[AudioSpeakerAnalysis] Parsed data:", result);
    return result;
  }, [data, rawText]);
  ...
  const totalDuration = useMemo(() => {
    ...
    if (parsedData.speakers.length > 0) {
      const maxTotalSeconds = Math.max(...parsedData.speakers.map(s => s.totalSeconds));
  ...
  return (
    <div className={styles.container}>
                ...
                <div className={styles.speakerCardContent}>
                  ...
                  <div className={styles.speakerStats}>
                    <div className={styles.speakerStat}>
                      ...
                      <span className={styles.statValue}>{speaker.totalTime}</span>
                    </div>
                    <div className={styles.speakerStat}>
                      <span className={styles.statLabel}>Percentage:</span>
                      <span className={styles.statValue}>{speaker.percentage}%</span>
                    </div>
      ...
      {(parsedData.timeline.length > 0 || parsedData.speakers.length > 0) && totalDuration > 0 && (
          ...
          <div className={styles.timelineContainer}>
            ...
            <div className={styles.timelineTracks}>
              {parsedData.speakers.map((speaker, speakerIndex) => {
                const speakerTimeline = timelineBySpeaker[speaker.label] || [];
                  ...
                  <div key={speakerIndex} className={styles.timelineTrack}>
                    <div className={styles.timelineTrackLabel}>
                      <div
                        ...
                        style={{ backgroundColor: getSpeakerColor(speakerIndex) }}
                      ></div>
                    ...
                    <div className={styles.timelineTrackBar}>
                      {hasTimelineData ? (
                        speakerTimeline.map((item, itemIndex) => {
                          const left = calculateTimelinePosition(item.startTime, totalDuration);
                          const width = calculateTimelineWidth(item.startTime, item.endTime, totalDuration);
                          return (
                            <div
                              key={itemIndex}
      ...
      {parsedData.speakers.length > 0 && (
        <div className={styles.durationChartSection}>
          ...
          <div className={styles.durationChart}>
            {parsedData.speakers.map((speaker, index) => {
              const maxPercentage = Math.max(...parsedData.speakers.map(s => s.percentage));
              ...
              <tbody>
                {parsedData.speakerBreakdown.map((speaker, index) => (
                  <tr key={index}>
                    ...
                    <td className={styles.percentageCell}>{speaker.percentage}%</td>
                    <td className={styles.segmentsCell}>{speaker.segments}</td>
                    <td className={styles.avgLengthCell}>{speaker.avgSegmentLength}</td>
